...
class CharacterWithEquipmentTest {
    ...
    @Test
    void getDPSWithWeaponAndArmour_ValidValue_ShouldPass() {
        double expectedDPS = (7 * 1.1) * (1 + ((5 + 1) / 100.0));

        try {
            testWarrior.equipItem(Slot.WEAPON, testWeapon);
            testWarrior.equipItem(Slot.BODY, testPlateBody);
        } catch (InvalidWeaponException | InvalidArmourException e) {
            e.printStackTrace();
        }
    ...
    @Test
    void returnTrueWhenEquippingValidArmour_ValidValue_ShouldPass() throws InvalidWeaponException, InvalidArmourException {
        boolean expectedResult = true;
        boolean actualResult = testWarrior.equipItem(Slot.BODY, testPlateBody);
